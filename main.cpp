...
const float LIGHT_ROTATE_X_MIN = -180.0;
const float LIGHT_ROTATE_X_MAX =  180.0;
const float LIGHT_ROTATE_Y_MIN = -180.0;
const float LIGHT_ROTATE_Y_MAX =  180.0;
const float LIGHT_ROTATE_Z_MIN = -180.0;
const float LIGHT_ROTATE_Z_MAX =  180.0;
const float LIGHT_TRANSLATE_X_MIN = -1.0;
const float LIGHT_TRANSLATE_X_MAX =  1.0;
const float LIGHT_TRANSLATE_Y_MIN = -1.0;
const float LIGHT_TRANSLATE_Y_MAX =  1.0;
const float LIGHT_TRANSLATE_Z_MIN = -1.0;
const float LIGHT_TRANSLATE_Z_MAX =  1.0;
...
// Functions to help draw the object, the render and draw
// functions are defined in parts.h
...
void renderHead();
void drawHead();
void renderUpperBeak();
void drawUpperBeak();
void renderLowerBeak();
void drawLowerBeak();
void renderEye();
void renderBody();
void drawBody();
void renderShoulderL();
void renderShoulderR();
void drawShoulder();
void renderElbowL();
void renderElbowR();
void drawElbow();
...
void initGlui()
{
	...
	//
	glui_joints = GLUI_Master.create_glui("Joint Control", 0, Win[0]+100, 0);
	...
	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "rotate around x:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::ROOT_ROTATE_X));
	glui_spinner->set_float_limits(ROOT_ROTATE_X_MIN, ROOT_ROTATE_X_MAX, GLUI_LIMIT_WRAP);
	glui_spinner->set_speed(SPINNER_SPEED);

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "rotate around y:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::ROOT_ROTATE_Y));
	glui_spinner->set_float_limits(ROOT_ROTATE_Y_MIN, ROOT_ROTATE_Y_MAX, GLUI_LIMIT_WRAP);
	glui_spinner->set_speed(SPINNER_SPEED);

	glui_spinner = glui_joints->add_spinner_to_panel(glui_panel, "rotate around z:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::ROOT_ROTATE_Z));
...
//    Light source GUI. Currently only rotations around x,
//    y, x are working

    glui_lightsource = GLUI_Master.create_glui("Light Source Control", 0, Win[0]+40, Win[1]+40);
    glui_panel = glui_lightsource->add_panel("Light Source Position");

    glui_spinner = glui_lightsource->add_spinner_to_panel(glui_panel, "rotate around x:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_ROTATE_X));
    glui_spinner->set_float_limits(LIGHT_ROTATE_X_MIN, LIGHT_ROTATE_X_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);

    glui_spinner = glui_lightsource->add_spinner_to_panel(glui_panel, "rotate around y:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_ROTATE_Y));
    glui_spinner->set_float_limits(LIGHT_ROTATE_Y_MIN, LIGHT_ROTATE_Y_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);
    ...
    glui_spinner->set_float_limits(LIGHT_ROTATE_Z_MIN, LIGHT_ROTATE_Z_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);

    glui_spinner = glui_lightsource->add_spinner_to_panel(glui_panel, "translate x:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_TRANSLATE_X));
    glui_spinner->set_float_limits(LIGHT_TRANSLATE_X_MIN, LIGHT_TRANSLATE_X_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);

    glui_spinner = glui_lightsource->add_spinner_to_panel(glui_panel, "translate y:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_TRANSLATE_Y));
    glui_spinner->set_float_limits(LIGHT_TRANSLATE_Y_MIN, LIGHT_TRANSLATE_Y_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);

    glui_spinner = glui_lightsource->add_spinner_to_panel(glui_panel, "translate z:", GLUI_SPINNER_FLOAT, joint_ui_data->getDOFPtr(Keyframe::LIGHT_TRANSLATE_Z));
    glui_spinner->set_float_limits(LIGHT_TRANSLATE_Z_MIN, LIGHT_TRANSLATE_Z_MAX, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);
...
void display(void)
{
    ...
    RenderInit(); // Initialize GL functions necessary
    RenderScene(); // Actual rendering
...
void RenderInit()
{
	...
	glMaterialfv(GL_FRONT,GL_SPECULAR,COLOR_DIFFUSE_DEFAULT);
	glMaterialfv(GL_FRONT,GL_SPECULAR,COLOR_SPECULAR_DEFAULT);
    ...
    glLineWidth(1.0f);
	if ((renderStyle == WIREFRAME) || (renderStyle == SOLID) || (renderStyle == OUTLINED))
	{
		glDisable(GL_LIGHTING);
        glDisable(GL_LIGHT0);
	}
    if ((renderStyle == SHINY) || (renderStyle == MATTE))
    {
...
void RenderScene()
{
	...
	glPushMatrix();
		...
		// setup transformation for the entire penguin
        ...
        GLfloat LightPosition[] = {float(cos(joint_ui_data->getDOF(Keyframe::LIGHT_ROTATE_Y))*cos(joint_ui_data->getDOF(Keyframe::LIGHT_ROTATE_Z))),
	                               float(sin(joint_ui_data->getDOF(Keyframe::LIGHT_ROTATE_Z))),
								   float(sin(joint_ui_data->getDOF(Keyframe::LIGHT_ROTATE_Y))),
                                   0.1f};
...
//------------------- Head Rendering Functions ---------------------------
// Renders the actual head
// head is in red color
void renderHead()
{
    glTranslatef(0.0f, 0.5f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::HEAD),
                    0.0, 0.0, 1.0);
    ...
    switch(renderStyle)
    {
        case WIREFRAME:
            ...
            break;
        case SOLID:
            glColor3fv(COLOR_WHITE); // Set color
        ...
        case OUTLINED:
            ...
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawHead();
            glColor3fv(COLOR_BLACK); // Set color
            glEnable(GL_LINE_SMOOTH);
            glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
            drawHead(); // draw head
            break;
        case SHINY:
            ...
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, COLOR_RED);
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, COLOR_RED);
            glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, SHININESS);
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawHead(); // draw head
			break;
        case MATTE:
            glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, COLOR_RED); // set diffuse material
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawHead(); // draw head
			break;
    }
...
//------------------- Beak Rendering Functions ---------------------------
void renderUpperBeak()
{
    glTranslatef(0.0f, joint_ui_data->getDOF(Keyframe::BEAK), 0.0f);
    ...
    glScalef(0.3f, 0.1f, 0.1f);
        ...
        switch(renderStyle)
        {
            case WIREFRAME:
                ...
                glEnable(GL_LINE_SMOOTH);
                glPolygonMode(GL_FRONT_AND_BACK,GL_LINE);
                drawUpperBeak(); // draw upper beak
                break;
            case SOLID:
                glColor3fv(COLOR_CYAN); // Set color
                glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
                drawUpperBeak(); // draw upper beak
                break;
            case OUTLINED:
			...
			case SHINY:
			    // set specular material
			    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, COLOR_CYAN);
    	        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, COLOR_CYAN);
    	        glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, SHININESS);
    	        glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
    	        drawUpperBeak(); // draw upper beak
				break;
	        case MATTE:
    	        glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, COLOR_CYAN); // set diffuse material
    	        glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
    	        drawUpperBeak(); // draw upper beak
				break;
        }
    ...
    glScalef(0.3f, 0.1f, 0.1f);
        ...
        switch(renderStyle)
        {
			...
			case SHINY:
				...
				break;
	        case MATTE:
    	        glMaterialfv(GL_FRONT_AND_BACK,GL_DIFFUSE,COLOR_CYAN); // set matte material
    	        glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
    	        drawLowerBeak(); // draw lower beak
				break;
        }
...
void renderBody()
{
    ...
    switch(renderStyle)
    {
        case WIREFRAME:
            ...
            break;
        case SOLID:
            glColor3fv(COLOR_ORANGE); // Set color
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawBody(); // draw body
            break;
        case OUTLINED:
        ...
        case SHINY:
            // set specular material
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, COLOR_ORANGE);
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, COLOR_ORANGE);
    	    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, SHININESS);
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawBody(); // draw body
			break;
        case MATTE:
            glMaterialfv(GL_FRONT_AND_BACK,GL_DIFFUSE,COLOR_ORANGE); // set diffuse material
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawBody(); // draw body
			break;
    }
...
//------------------- Shoulder Rendering Function ---------------------------
void renderShoulderL()
{

    glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_PITCH),
              0.0, 0.0, 1.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_YAW),
              0.0, 1.0, 0.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_ROLL),
              1.0, 0.0, 0.0);
...
void renderShoulderR()
{
    glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_PITCH),
              0.0, 0.0, 1.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW),
              0.0, 1.0, 0.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_ROLL),
              1.0, 0.0, 0.0);
    glRotatef(3.0f, 1.0, 0.0, 0.0);
    glTranslatef(0.0f, 0.0f, -0.5f);
    ...
    switch(renderStyle)
    {
		...
		case SHINY:
			...
			break;
        case MATTE:
            glMaterialfv(GL_FRONT_AND_BACK,GL_DIFFUSE,COLOR_CYAN);
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawShoulder(); // draw head
			break;
    }
...
//------------------- Elbow Rendering Function ---------------------------
void renderElbowL()
{
    glTranslatef(-0.05f, -1.0f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_ELBOW),
              0.0, 0.0, 1.0);
    ...
    switch(renderStyle)
    {
        ...
        case OUTLINED:
            ...
            break;
		case SHINY:
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, COLOR_BLUE);
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, COLOR_BLUE);
    	    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, SHININESS);
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawElbow(); // draw head
			break;
...
{
    glTranslatef(-0.05f, -1.0f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_ELBOW),
              0.0, 0.0, 1.0);
    glScalef(0.2f, 0.3f, 0.1f);
    glTranslatef(0.0f, -1.0f, 0.0f);
    // determine render style and set glPolygonMode appropriately
...
//------------------- Hip Rendering Function ---------------------------
void renderHipL()
{

    glTranslatef(0.0f, -1.3f, 0.4f);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_PITCH),
              0.0, 0.0, 1.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_YAW),
              0.0, 1.0, 0.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_ROLL),
              1.0, 0.0, 0.0);
    ...
    switch(renderStyle)
    {
		...
		case SHINY:
            ...
            glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, COLOR_GREEN);
    	    glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, SHININESS);
            glPolygonMode(GL_FRONT_AND_BACK,GL_FILL);
            drawHip(); // draw head
			break;
...
void renderHipR()
{
    // Controlled transformations and group positioning of
//     the right hip and right knee
    glTranslatef(0.0f, -1.3f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_PITCH),
              0.0, 0.0, 1.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_YAW),
              0.0, 1.0, 0.0);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_HIP_ROLL),
              1.0, 0.0, 0.0);

    // Drawing and Positioning of the right hip and right knee
...
//------------------- Knee Rendering Function ---------------------------
void renderKneeL()
{
    glTranslatef(0.1f, -0.4f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::L_KNEE),
              0.0, 0.0, 1.0);
...
{
    glTranslatef(0.1f, -0.4f, 0.0f);
    glRotatef(joint_ui_data->getDOF(Keyframe::R_KNEE),
              0.0, 0.0, 1.0);
    glScalef(0.4f, 0.05f, 0.3f);
    glTranslatef(-1.0f, -1.0f, 0.0f);
...
void writePGM(char* filename, GLubyte* buffer, int width, int height, bool raw=true)
{
	...
	else
	{
		...
		for(int y=height-1;y>=0;y--)
		{
			for(int x=0;x<width;x++)
			{
//				fprintf(fp,"%c ", char(&buffer[x+y*width]));
			}
			fprintf(fp,"\n");
		}
	}
